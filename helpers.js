...
const validations = {
    ...
    checkGender (strVal, varName)
    {
        ...
        if(strVal === "male" || strVal === "female" || strVal === "prefer not to say")
            return strVal;
    ...
    checkState (strVal, varName)
    {
        ...
        if(!isValidState(strVal))               // using api to check is the valid state in the US
    ...
    checkExperience (strVal, varName)
    {
        ...
        if(isNaN(strVal)) throw `Error: ${varName} must supply a number that is not NaN!`;
        if(strVal < 0 || strVal > 80) throw `Error: ${varName} can not lower than 0 or greater than 80 years!`;
        return strVal
    },
    ...
    checkGroupAndConnections (arr, varName)         //Different from checkStringArray. This function will not only check the array elements contain non-string elements but take care of the string is object id or not.
    {
        //We will allow an empty array for this,
        //if it's not empty, we will make sure all tags are strings
        if(!arr || !Array.isArray(arr))
    ...
    checkValidDate (strVal, varName)  // valid date form DD/MM/YYYY
    {
        if(!strVal) throw `Error: You must supply a ${varName}!`;
        if(typeof strVal !== 'string') throw `Error: ${varName} must be a string!`;
        strVal = strVal.trim();
