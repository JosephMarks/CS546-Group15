...
const exportedMethods = {
    ...
    {
        const skillsCollection = await skills();
        const skills = await skillsCollection.find({ skillsId: skillsId }).toArray();
    ...
    async createSkills (userId, title, article, videoUrl, tags)
    {
        ...
        const returnSkills = await this.getSkillsBySkillsId(newSkillsInfo._id.toString());
    ...
    async updateSkills (skillsId, userId, title, article, videoUrl, tags)
    {
        ...
        if(updateInfo.lastErrorObject.n === 0)`Error: Update failed, could not find a match skills article with skillsId: ${skillsId} or userId: ${userId}!`
    ...
    async removeSkills (skillsId, userId)
    {
        ...
        userId = validations.checkId(userId);
        const skillsCollection = await skills();
        const deletionInfo = await skillsCollection.findOneAndDelete({ _id: new ObjectId(postId), userId: userId });
