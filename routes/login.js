...
router.route('/data').post(async (req, res) => {
    ...
    try {
        ...
        if (!validations.isProperString([email, pass])) throw "Error : Email and Password can only be string not just string with empty spaces"
    ...
    try {
        ...
        if (e === "Error: User Email is not registered") return res.status(400).json({ errorMessage: e})
        if (e === "Error : Wrong Password") return res.status(400).json({ errorMessage: e})
