...
router.route("/data").post(async (req, res) =>
{
  ...
  try {
    ...
    if (! rules.validate(pass)) throw "Error : Password Must be atleast 8 characters with a capital letter, number, and special character";
  ...
  try
  {
    ...
    return res.render('welcome', { message: `You are Logged In as ${email}}`, title: 'Welcome' });
  } catch(e)
  {
    if(e === "Error : Email is not registered or Wrong Password") return res.status(400).render('Auth/login', { error: e });
    else return res.status(500).render('error', { error: "Server Error", title: 'Error' });
  }
});
